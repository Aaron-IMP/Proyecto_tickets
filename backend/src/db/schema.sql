...
CREATE TABLE reventa (
  ...
  vendedor_id       INT REFERENCES usuario(id),
  comprador_id      INT REFERENCES usuario(id),
  precio_reventa    DECIMAL(10,2) NOT NULL,
  estado            estado_reventa DEFAULT 'publicado',
...
CREATE TYPE metodo_pago  AS ENUM ('tarjeta', 'yape', 'plin');

CREATE TABLE pago (
  id               SERIAL PRIMARY KEY,
  referencia       UUID DEFAULT gen_random_uuid() UNIQUE NOT NULL,
  usuario_id       INT REFERENCES usuario(id),
  monto            DECIMAL(10,2) NOT NULL,
  metodo           metodo_pago NOT NULL,
  estado           estado_pago DEFAULT 'pendiente',
  -- Para tarjeta: últimos 4 dígitos enmascarados
  ultimos_digitos  CHAR(4),
  -- Opcional: número de teléfono para Yape/Plin
  telefono         VARCHAR(15),
  -- El ticket asociado (NULL hasta que el pago se aprueba)
